...
public class MetaData {
    ...
    public static Future<MetaData> create(ConfigurationRegistry configurationRegistry, @Nullable String ephemeralId) {
        ...
        final boolean runsOnLambda = configurationRegistry.getConfig(ServerlessConfiguration.class).runsOnAwsLambda();
        final ServiceFactory serviceFactory = new ServiceFactory();
        final Service service = serviceFactory.createService(coreConfiguration, ephemeralId, runsOnLambda);
        ...
        if (runsOnLambda) {
            return createMetadataWithAwsLambdaCloudProvider(coreConfiguration, processInformation, service, system);
        } else if (cloudProvider == CoreConfiguration.CloudProvider.NONE) {
            ...
            return createMetadataWithCloudProvider(coreConfiguration, processInformation, service, system);
        }
    }

    private static Future<MetaData> createMetadataWithAwsLambdaCloudProvider(CoreConfiguration coreConfiguration, ProcessInfo processInformation,
                                                                             Service service, SystemInfo system) {
        CloudProviderInfo cloudProviderInformation = new CloudProviderInfo("aws");
        cloudProviderInformation.setRegion(System.getenv("AWS_REGION"));
        cloudProviderInformation.setService(new CloudProviderInfo.Service("lambda"));
        ...
        return new EditableMetadataFuture(metaData);
    }

    private static Future<MetaData> createMetadataWithCloudProvider(final CoreConfiguration coreConfiguration, final ProcessInfo processInformation,
                                                                    final Service service, final SystemInfo system) {
        ...
        try {
            return executor.submit(new Callable<MetaData>() {
                ...
                public MetaData call() {
                    ...
                    final CloudProviderInfo cloudProviderInfo = CloudMetadataProvider.fetchAndParseCloudProviderInfo(coreConfiguration.getCloudProvider(), cloudDiscoveryTimeoutMs);
                    return new MetaData(
                        processInformation,
                        service,
                        system,
    ...
    /**
     * A specific future for metadata that allows editing the metadata object
     * and explicitly setting it to a ready state for serialization.
     */
    public static class EditableMetadataFuture implements Future<MetaData> {

        private final MetaData metaData;

        private boolean ready = false;

        public EditableMetadataFuture(MetaData metaData) {
            this.metaData = metaData;
        }

        public MetaData getEditableMetaData(){
            return metaData;
        }
        ...
        public MetaData get() {
            if(!ready){
                throw new CancellationException("Metadata not ready for serialization, yet.");
            }
            return metaData;
        }
