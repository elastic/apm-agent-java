...
public class HeaderGetterBridge<T, C> implements HeaderGetter<T, C> {
    ...
    private final co.elastic.apm.tracer.api.dispatch.HeaderGetter<T, C> headerGetter;

    public HeaderGetterBridge(co.elastic.apm.tracer.api.dispatch.HeaderGetter<T, C> headerGetter) {
    ...
    @Override
    public <S> void forEach(String headerName, C carrier, S state, co.elastic.apm.tracer.api.dispatch.HeaderGetter.HeaderConsumer<T, S> consumer) {
