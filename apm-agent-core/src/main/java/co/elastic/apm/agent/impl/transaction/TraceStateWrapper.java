...
/**
 * Implementation of {@link TraceStateImpl} that allows to wrap an existing {@link TraceStateImpl} object in order to
 ...
 * </ul>
 * <p>
 *     When such cases occur, the currently active {@link TraceStateImpl} in {@link co.elastic.apm.agent.impl.ElasticApmTracer}
 ...
 * </p>
 * <p>
 *       This replacement is transparent to the creator of the original active {@link TraceStateImpl}. Calling (and
...
public class TraceStateWrapper<T extends TraceStateImpl<T>> extends TraceStateImpl<T> {
    ...
    public TraceStateWrapper(int initialSize, TraceStateImpl<T> context) {
     ...
     */
    public <C extends TraceStateImpl<C>> C wrapIfRequired(Class<C> wrapperType, Callable<C> wrapFunction) {
        TraceStateImpl<?> wrapped = contextWrappers.get(wrapperType);
        if (wrapped == null) {
            try {
