...
public class CallDepth {
    ...
    /**
     * Calls {@link #increment(Class)} and returns {@code false} if this is the outer-most (non-nested) invocation.
     *
     * @param clazz the class for which the call depth should be counted.
     *              Used as a key to distinguish multiple counters for a thread.
     * @return {@code false} if this is the outer-most (non-nested) invocation, {@code true} otherwise
     */
    public static boolean isNestedCallAndIncrement(Class<?> clazz) {
        return increment(clazz) != 0;
    }
    ...
    /**
     * Calls {@link #decrement(Class)} and returns {@code false} if this is the outer-most (non-nested) invocation.
     *
     * @param clazz the class for which the call depth should be counted.
     *              Used as a key to distinguish multiple counters for a thread.
     * @return {@code false} if this is the outer-most (non-nested) invocation, {@code true} otherwise
     */
    public static boolean isNestedCallAndDecrement(Class<?> clazz) {
        return decrement(clazz) != 0;
    }
}
