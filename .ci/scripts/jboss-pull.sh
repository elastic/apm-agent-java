...
while read -r i
do
  docker pull docker.elastic.co/observability-ci/$i --platform linux/amd64
  docker tag docker.elastic.co/observability-ci/$i registry.access.redhat.com/jboss-eap-7/$i 
