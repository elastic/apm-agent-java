...
If you wish to use a locally built artifact in the built image, execute `./mvnw package`
and ensure that artifacts are present in `elastic-apm-agent/target/*.jar`. To create
a Docker image from artifacts generated by `./mvnw package`, simply run [`scripts/jenkins/build_docker.sh`](scripts/jenkins/build_docker.sh).
...
To do so, first clean any artifacts with `./mvnw clean` and then run the Docker
...
After running the `build_docker.sh` script, images can be seen by executing
...
and that the output of the `build_docker.sh` script should be examined to determine
the cause.


#### Pushing a image to the Elastic repo

*Notice:* You must have access to release secrets in order to push images.
