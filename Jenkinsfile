...
pipeline {
  ...
  parameters {
    string(name: 'MAVEN_CONFIG', defaultValue: '-B -Dorg.slf4j.simpleLogger.log.org.apache.maven.cli.transfer.Slf4jMavenTransferListener=warn -Dhttps.protocols=TLSv1.2 -Dmaven.wagon.http.retryHandler.count=3 -Dhttp.keepAlive=false -Dmaven.wagon.http.pool=false', description: 'Additional maven options.')
        ...
        stage('Build') {
          ...
          steps {
            withGithubNotify(context: 'Build', tab: 'artifacts') {
              ...
              unstash 'source'
              dir("${BASE_DIR}"){
                sh label: 'mvn install', script: "./mvnw -V clean install -DskipTests=true -Dmaven.javadoc.skip=true"
