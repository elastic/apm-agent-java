...
pipeline {
  ...
  environment {
    ...
    MAVEN_CONFIG = '-Dmaven.repo.local=.m2 -s .ci/settings.xml -Pci'
  ...
  post {
    cleanup {
      notifyBuildResult(analyzeFlakey: !isTag(), flakyReportIdx: 'reporter-apm-agent-java-apm-agent-java-master')
    }
  }
}
