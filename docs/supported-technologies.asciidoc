...
|
|The agent instruments any method annotated with one of the following:  
...
|Quartz
|
|The agent instruments the `execute` method of any class implementing `org.quartz.Job`, as well as the `executeInternal` method of any class extending `org.springframework.scheduling.quartz.QuartzJobBean`, and creates a transaction with the type `quartz`, representing the job execution 
|1.8.0 - Incubating (off by default). In order to enable, set the <<config-disable-instrumentations>> config option to an empty string
|===
