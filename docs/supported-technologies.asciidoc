...
|RocketMQ
|4.X
|The agent captures RocketMQ Message sends and consumes.  When the producer send message within a traced transaction, a span will be created.
In addition, the agent will create transactions as the push consumer uses a defined listener to consume messages or read each message from the results returned by the pull consumer.
|1.13.1
