...
|1.7.0 - Incubating (off by default). Use <<config-disable-instrumentations>> to enable
...
side will be a part of the same trace containing the producer side as well.
|1.13.0
...
| 1.13.0 - Incubating (off by default). Use <<config-disable-instrumentations>> to enable
...
|Easy to overuse by matching too many methods which hurts performance. Use together with <<config-trace-methods-duration-threshold, `trace_methods_duration_threshold`>> if setting for entire packages instead of specific methods in order to reduce overhead and avoid having too many spans in the APM app.
