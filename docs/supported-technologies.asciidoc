...
|Annotations
|Annotate any method with <<api-capture-span>> or <<api-capture-transaction>>.
...
|Configuration
|Use the <<config-trace-methods>> configuration option to specify additional methods to instrument.
...
|Easy to overuse by matching too many methods which hurts performance.
