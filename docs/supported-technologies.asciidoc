...
- *Log correlation*:
    The agent automatically injects `trace.id`, `transaction.id` and `error.id` into MDC implementation when a log event
    is written to log. See <<log-correlation, Log Correlation>> for details.
    ...
    with <<config-log-ecs-reformatting, `log_ecs_reformatting`>>.

- *Error capturing*:
    Automatically captures exceptions for calls like `logger.error("message", exception)`.
    When doing so, the ID corresponding the captured error (`error.id`) is added to the MDC as well for log correlation
    since 1.16.0.
...
- *ECS Service Name*: The agent sets the service name and version for the `EcsLayout` if not provided explicitly.
...
|The agent adds https://logging.apache.org/log4j/2.x/manual/thread-context.html[ThreadContext] entries for correlation.

The agent sets the service name and version for the `EcsLayout` if not provided explicitly (since 1.29.0).
...
|The agent adds https://logging.apache.org/log4j/1.2/apidocs/org/apache/log4j/MDC.html[MDC] entries for correlation.
...
|Logback
|1.1.0+
|The agent adds https://www.slf4j.org/api/org/slf4j/MDC.html[slf4j MDC] entries for correlation.
