...
|cdi
|1.0, 1.2, 2.0
|When Classes annotated with CDI scope annotation are detected in a package configured in <<config-application-packages>>,
the agent will add a span for each public method called on those classes.
Short running method calls can be excluded via the <<config-cdi-methods-duration-threshold>> option.
