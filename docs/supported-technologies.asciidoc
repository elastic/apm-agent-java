...
|Quartz
|
|The agent instruments the `execute` method of any class implementing org.quartz.Job. It also instruments the `executeInternal` method of any class with the supertype of `org.springframework.scheduling.quartz.QuartzJobBean`
|1.8.0 - Incubating (off by default). In order to enable, set the <<config-disable-instrumentations>> config option to an empty string
|===