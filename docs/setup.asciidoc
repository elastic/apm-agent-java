...
In some cases, the agent needs to take a decision to abort very early, before almost any initialization occurs.
For example, when it is attached to a non-supported JVM version.
If any of the bootstrap checks fails, the agent will log an error to the standard error stream and abort.
It is possible to disable bootstrap checks through the `elastic.apm.disable_bootstrap_checks` System property, or the
...
`(?-i)}` makes the matching case-sensitive.
