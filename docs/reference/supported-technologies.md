...
| --- | --- | --- |
| Oracle JDK | 7u60+*, 8u40+, 9, 10, 11, 17, 21 | `--module-path` has not been tested yet |
| OpenJDK | 7u60+*, 8u40+, 9, 10, 11, 17, 21 | `--module-path` has not been tested yet |
...
| Spring Webflux | 5.2.3+ | Creates transactions for incoming HTTP requests, supports annotated and functional endpoints. | 1.24.0 (experimental), 1.34.0 (GA), 6.1+ since 1.45.0 |
