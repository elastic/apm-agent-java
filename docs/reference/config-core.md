...
Individual dropped spans are not reported as trace data, but their self-times are still included in breakdown metrics (`span.self_time`). See [Built-in application metrics](/reference/metrics.md#metrics-application) for details.
