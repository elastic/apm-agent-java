...
Some flags, like `--include-cmd` and `--exclude-cmd` require the `jps` command to be installed,
...
By default, this program will not attach the agent to any JVM unless any condition is true.
...
NOTE: this is only available if jps is installed
--
