...
[float]
[[config-allow-path-on-hierarchy]]
==== `allow_path_on_hierarchy`

According to JAX-RS 2.0 spec (section 3.6 Annotation Inheritance) inheritance of class or interface annotations is not supported.
...
|============
| Java System Properties      | Property file   | Environment
| `elastic.apm.allow_path_on_hierarchy` | `allow_path_on_hierarchy` | `ELASTIC_APM_ALLOW_PATH_ON_HIERARCHY`
|============
