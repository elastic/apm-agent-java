...
Configure one of the following methods (listed in descending order of precedence) to adapt the Elastic APM Java agent to your needs.
...
<<config-server-url>> and <<config-application-packages>> for Elastic APM.
A minimal version of a configuration might look like this:
