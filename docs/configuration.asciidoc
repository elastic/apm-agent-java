...
* <<config-jdbc>>
** <<config-use-jdbc-service-resource-auto-inference>>
* <<config-jmx>>
...
[float]
[[config-use-jdbc-service-resource-auto-inference]]
==== `use_jdbc_service_resource_auto_inference` (added[1.25.0])

If set to `true`, the agent uses `type`, `subtype` and `db.instance` of a JDBC span to infer its `destination.service.resource` instead of relying on the JDBC instrumentation to set it.
...
|============
| Java System Properties      | Property file   | Environment
| `elastic.apm.use_jdbc_service_resource_auto_inference` | `use_jdbc_service_resource_auto_inference` | `ELASTIC_APM_USE_JDBC_SERVICE_RESOURCE_AUTO_INFERENCE`
|============
...
# Default value: false
#
# use_jdbc_service_resource_auto_inference=false
