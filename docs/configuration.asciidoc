...
[float]
[[config-log-shading-replace]]
==== `log_shading_replace` (performance)
...
|============
| Java System Properties      | Property file   | Environment
| `elastic.apm.log_shading_replace` | `log_shading_replace` | `ELASTIC_APM_LOG_SHADING_REPLACE`
|============
...
As long as <<config-log-shading-override,`log_shading_override`>> is set to `false`, the shade 
