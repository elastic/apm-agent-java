...
via configuration sources which support dynamic reloading. The `elasticapm.properties` will be scanned
for changes every minute and reloaded if required(does not work for config on classpath). Java system properties can be dynamic as well by being set from within the application.
