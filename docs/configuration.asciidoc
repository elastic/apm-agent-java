...
** <<config-use-annotation-value-for-transaction-name>>
* <<config-http>>
** <<config-capture-body>>
...
[float]
[[config-use-annotation-value-for-transaction-name]]
==== `use_annotation_value_for_transaction_name` (internal)

By default, the agent will use ClassName#methodName for transaction name of JAX-RS.
If you need use in transaction name value from @javax.ws.rs.Path annotation, you should set value to 'true'.
...
|============
| Java System Properties      | Property file   | Environment
| `elastic.apm.use_annotation_value_for_transaction_name` | `use_annotation_value_for_transaction_name` | `ELASTIC_APM_USE_ANNOTATION_VALUE_FOR_TRANSACTION_NAME`
|============
