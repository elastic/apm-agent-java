...
[float]
[[config-circuit-breaker-enabled]]
==== `circuit_breaker_enabled` (added[1.14.0])
...
If ALL monitors approve that the system/process/JVM is not under stress anymore, the agent will resume and become fully functional.
...
[float]
[[config-stress-monitoring-interval]]
==== `stress_monitoring_interval` (added[1.14.0])

The interval at which the agent polls the stress monitors.
Must be at least `1s`.
