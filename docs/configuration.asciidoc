...
* <<config-cdi>>
** <<config-cdi-methods-duration-threshold>>
...
If <<config-trace-methods, `trace_methods`>> config option is set, provides a threshold to limit spans based on duration.
When set to a value greater than 0, spans representing methods traced based on `trace_methods` will be discarded by default.
...
NOTE: Transaction are never discarded, regardless of their duration.
This configuration affects only spans.
In order not to break span references, all spans leading to an async operations are never discarded, regardless of their duration.
...
[[config-cdi]]
=== cdi configuration options

[float]
[[config-cdi-methods-duration-threshold]]
==== `cdi_methods_duration_threshold` (added[1.7.0])

Provides a threshold to limit spans generated by the cdi-plugin based on duration.
When set to a value greater than 0, spans representing methods traced by the cdi plugin will be discarded by default.
Such methods will be traced and reported if one of the following applies:
- This method's duration crossed the configured threshold.
- This method ended with Exception.
- A method executed as part of the execution of this method crossed the threshold or ended with Exception.
...
|============
| Java System Properties      | Property file   | Environment
| `elastic.apm.cdi_methods_duration_threshold` | `cdi_methods_duration_threshold` | `ELASTIC_APM_CDI_METHODS_DURATION_THRESHOLD`
|============
...
# Provides a threshold to limit spans generated by the cdi-plugin based on
# duration. When set to a value greater than 0, spans representing methods traced by the cdi plugin will be discarded by default.
# Such methods will be traced and reported if one of the following applies:
#  - This method's duration crossed the configured threshold.
#  - This method ended with Exception.
#  - A method executed as part of the execution of this method crossed the threshold or ended with Exception.
...
# Default value: 0ms
#
# cdi_methods_duration_threshold=0ms
