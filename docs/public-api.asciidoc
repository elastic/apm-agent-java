...
[float]
[[api-start-transaction-with-remote-parent-map]]
==== `Transaction startTransactionWithRemoteParent(Map<String, ? extends Iterable<String>> headers)`
Similar to <<api-start-transaction>> but creates this transaction as the child of a remote parent.

* `headers`: a map, representing the headers of the incoming request

NOTE: If you are using Java 8 or newer, it is recommended to use <<api-start-transaction-with-remote-parent-function>>

since 1.3.0

[float]
[[api-start-transaction-with-remote-parent-function]]
==== `Transaction startTransactionWithRemoteParent(Function<String, String> getFirstHeader)`
Similar to <<api-start-transaction>> but creates this transaction as the child of a remote parent.

* `getFirstHeader`:  a function which receives a header name and returns the fist header with that name
...
NOTE: If the protocol supports multi-value headers, use <<api-start-transaction-with-remote-parent-function-function>>

NOTE: This method can only be used on Java 8+.
If you are stuck on Java 7, use <<api-start-transaction-with-remote-parent-map>>.

since 1.3.0

[float]
[[api-start-transaction-with-remote-parent-function-function]]
==== `Transaction startTransactionWithRemoteParent(Function<String, String> getFirstHeader, Function<String, Iterable<String>> getAllHeaders)`

Similar to <<api-start-transaction>> but creates this transaction as the child of a remote parent.

* `getFirstHeader`:  a function which receives a header name and returns the fist header with that name
* `getAllHeaders`:   a function which receives a header name and returns all headers with that name
