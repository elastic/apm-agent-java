...
[float]
[[api-get-service-info-for-class-loader]]
==== `ServiceInfo getServiceInfoForClassLoader(ClassLoader)` added[1.29.0]
Gets the service name and version that is associated with a class loader

* `classLoader`: the class loader for which the service name and version should be looked up

[float]
[[api-set-service-info-for-class-loader]]
==== `void setServiceInfoForClassLoader(ClassLoader, ServiceInfo)` added[1.29.0]
...
* `serviceInfo`: the service name and version
...
==== `Transaction setServiceInfo(ServiceInfo serviceInfo)` added[1.29.0]
