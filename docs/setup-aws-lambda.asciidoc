...
Follow the <<aws-lambda-java-quick-start>> Guide to get started in minutes and checkout the <<aws-lambda-features-and-caveats>> to learn about the features and limitations of the Java APM Agent on AWS Lambda Functions.

[float]
[[aws-lambda-java-quick-start]]
==== Quick Start
include::{apm-aws-repo-dir}/add-extension/code.asciidoc[]

Before starting with the setup make sure you are using one of the  <<supported-aws-lambda-runtimes, supported AWS Lambda runtimes>>.
...
include::{apm-aws-repo-dir}/lambda-selector/lambda-attributes-selector.asciidoc[]

[float]
===== Step 2: Add the APM Layers to your Lambda function

include::{apm-aws-repo-dir}/lambda-selector/extension-arn-replacement.asciidoc[]
include::./lambda/java-arn-replacement.asciidoc[]

Both, the APM Lambda Extension and the Java APM Agent are added to your Lambda function as https://docs.aws.amazon.com/lambda/latest/dg/invocation-layers.html[AWS Lambda Layers]. Therefore, you need to add the corresponding Layer ARNs (identifiers) to your Lambda function.

include::{apm-aws-repo-dir}/add-extension/add-extension-layer-widget.asciidoc[]
...
For the minimal configuration, you will need the _APM Server URL_ to set the destination for APM data and an **{apm-guide-ref}/secret-token.html[APM Secret Token]**.
If you prefer to use an APM API key instead of the APM secret token, use the `ELASTIC_APM_API_KEY` environment variable instead of `ELASTIC_APM_SECRET_TOKEN` in the following configuration.
...
The AWS Lambda as a runtime behaves different with respect to some aspect compared to conventional runtimes.
While most APM and monitoring concepts are applicable to AWS Lambda, there are also a few limitations and specifics.
Read on to learn more about the Java APM features and caveats on AWS Lambda.
