...
`cf cups my-elastic-apm-service -p '{"server_urls":"my-apm-server-url","secret_token":"my-apm-server-secret-token"}'`

Both `my-apm-server-url` and `my-apm-server-secret-token` are respectively `server_urls` and `secret_token` from service-key of your Elasticsearch server.
