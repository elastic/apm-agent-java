...
[[setup-cloud-foundry]]
==== Cloud Foundry
Inject the `elastic-apm-agent-<version>.jar` into the root path of the deployable application, e.g. `my-application.jar`.
Update the `manifest.yml` file to add the `-javaagent` flag and system properties as part of `JAVA_OPTS`.
