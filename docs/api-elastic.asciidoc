...
[float]
[[api-span-set-remote-address]]
==== `Transaction setRemoteAddress(String remoteAddress)` added[1.31.0]
Provides a way to manually set the `http.request.socket.remote_address` field.
Using `null` or empty remote address will result in the omission of the corresponding field from the
transaction context.

[source,java]
----
transaction.setRemoteAddress(inetAddress.getHostAddress());
----
