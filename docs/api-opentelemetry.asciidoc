...
[source,java]
----
Span.current().setAttribute("elastic.otel.nondiscardable", true);
----

The `co.elastic.apm.agent.opentelemetry.sdk.BehavioralAttributes` utility class provides supported attributes as keys. Using them is
safer than using raw strings as attribute keys.

[float]
[[otel-config-set-non-discardable]]
===== `elastic.otel.nondiscardable`

Makes a span non-discardable. In some cases, spans may be discarded, for example if <<config-span-min-duration>> is set and the span
does not exceed the configured threshold. Use this attribute to make sure the current span is not discarded.
...
| `elastic.otel.nondiscardable` | Any | Any
|============
