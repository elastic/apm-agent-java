...
. <<opentelementry-bridge>> +
  ...
  Be aware that the API is not stable if you are using non GA (0.x) versions.
  In non GA versions, breaking changes are frequently introduced which means you may have to adapt your instrumentation code when updating.
. <<opentracing-bridge>> +
...
include::./api-elastic.asciidoc[]
include::./api-opentelemetry.asciidoc[]
include::./api-opentracing.asciidoc[]
