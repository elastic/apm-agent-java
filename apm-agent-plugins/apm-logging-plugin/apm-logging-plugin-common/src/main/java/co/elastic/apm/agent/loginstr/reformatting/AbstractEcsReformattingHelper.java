...
public abstract class AbstractEcsReformattingHelper<A, B, F, L> {
    ...
    public AbstractEcsReformattingHelper() {
        tracer = GlobalTracer.get().require(ReportingTracer.class);
        ...
        globalServiceName = config.getServiceName();
        globalServiceVersion = config.getServiceVersion();
        String configuredServiceNodeName = config.getServiceNodeName();
        if (configuredServiceNodeName == null) {
            ServerlessConfiguration serverlessConfiguration = tracer.getConfig(ServerlessConfiguration.class);
            this.configuredServiceNodeName = serverlessConfiguration.awsLambdaLogStreamName();
        } else {
