...
class LogbackEcsReformattingHelper extends AbstractEcsReformattingHelper<OutputStreamAppender<ILoggingEvent>, Appender<ILoggingEvent>, Encoder<ILoggingEvent>, ILoggingEvent> {
    ...
    @Override
    protected boolean isAllowedFormatter(Encoder<ILoggingEvent> formatter, List<? extends WildcardMatcher> allowList) {
        if (formatter instanceof LayoutWrappingEncoder<?>) {
            return WildcardMatcherUtil.anyMatch(allowList, ((LayoutWrappingEncoder<?>) formatter).getLayout().getClass().getName()) != null;
        }
    ...
    protected OutputStreamAppender<ILoggingEvent> createAndStartEcsAppender(OutputStreamAppender<ILoggingEvent> originalAppender,
        ...
        if (originalAppender instanceof FileAppender) {
            ...
            try {
                ...
                logger.info("Failed to set max file size for log ECS-reformatting file-rolling strategy. Using the default " +
