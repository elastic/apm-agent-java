...
public class ExceptionHandlerInstrumentationWithResponseStatusExceptionTest {
    ...
    public void testCallApiWithExceptionThrown() throws Exception {
        ...
        this.mockMvc.perform(get("/response-status-exception"));

        assertEquals(1, reporter.getTransactions().size());
        assertEquals(0, reporter.getSpans().size());
        assertEquals(1, reporter.getErrors().size());
        assertEquals("Response status 409 with reason \"responseStatusException\"; nested exception is java.lang.RuntimeException: runtime exception occured", reporter.getErrors().get(0).getException().getMessage());
