...
import co.elastic.apm.agent.spring.webmvc.exception.testapp.exception_handler.ExceptionHandlerRuntimeException200;
...
public class ExceptionHandlerInstrumentationWithExceptionHandlerTest extends AbstractExceptionHandlerInstrumentationTest {
    ...
    public void testExceptionCapture_IgnoreException() throws Exception {
        when(config.getConfig(CoreConfiguration.class).getIgnoreExceptions()).thenReturn(
            List.of(WildcardMatcher.valueOf("co.elastic.apm.agent.spring.webmvc.exception.testapp.exception_handler.ExceptionHandlerRuntimeException"))
        );
