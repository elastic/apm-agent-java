...
public class ResponseListenerWrapper2 extends ResponseListenerWrapper implements ResponseListener, Recyclable {
    ...
    ResponseListenerWrapper2(ElasticsearchRestClientInstrumentationHelper helper, Tracer tracer) {
        super(helper);
        this.helper = helper;
        this.tracer = tracer;
    }
    ...
    ResponseListenerWrapper2 withContextPropagation(ResponseListener delegate, AbstractSpan<?> context) {
        // Order is important due to visibility - write to span last on this (initiating) thread
        this.delegate = delegate;
