...
import co.elastic.apm.agent.servlet.JakartaEEAsyncInstrumentation;
import co.elastic.apm.agent.servlet.JakartaEEServletApiAdvice;
...
public class JakartaEEAsyncContextAdviceHelperImpl implements JakartaEEAsyncInstrumentation.JakartaEEAsyncContextAdviceHelper<AsyncContext> {

    private static final String ASYNC_LISTENER_ADDED = JakartaEEServletApiAdvice.class.getName() + ".asyncListenerAdded";
    private static final int MAX_POOLED_ELEMENTS = 256;

    private final ObjectPool<JakartaEEApmAsyncListener> asyncListenerObjectPool;
    private final ServletTransactionHelper servletTransactionHelper;
    private final Tracer tracer;

    public JakartaEEAsyncContextAdviceHelperImpl(ElasticApmTracer tracer) {
        this.tracer = tracer;
        servletTransactionHelper = new ServletTransactionHelper(tracer);

        asyncListenerObjectPool = QueueBasedObjectPool.ofRecyclable(
            AtomicQueueFactory.<JakartaEEApmAsyncListener>newQueue(createBoundedMpmc(MAX_POOLED_ELEMENTS)),
            false,
            new JakartaEEApmAsyncListenerAllocator());
    }
    ...
    private final class JakartaEEApmAsyncListenerAllocator implements Allocator<JakartaEEApmAsyncListener> {
        @Override
        public JakartaEEApmAsyncListener createInstance() {
            return new JakartaEEApmAsyncListener(JakartaEEAsyncContextAdviceHelperImpl.this);
        }
    }
    ...
    void recycle(JakartaEEApmAsyncListener apmAsyncListener) {
