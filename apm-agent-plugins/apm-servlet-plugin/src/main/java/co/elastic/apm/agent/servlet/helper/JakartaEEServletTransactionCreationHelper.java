...
public class JakartaEEServletTransactionCreationHelper {

    private static final Logger logger = LoggerFactory.getLogger(JakartaEEServletTransactionCreationHelper.class);

    private final Tracer tracer;
    private final WebConfiguration webConfiguration;

    public JakartaEEServletTransactionCreationHelper(ElasticApmTracer tracer) {
        this.tracer = tracer;
    ...
    public Transaction createAndActivateTransaction(HttpServletRequest request) {
        ...
        if (tracer.currentTransaction() == null &&
            ...
            transaction = tracer.startChildTransaction(request, JakartaEEServletRequestHeaderGetter.getInstance(), request.getServletContext().getClassLoader());
            if (transaction != null) {
