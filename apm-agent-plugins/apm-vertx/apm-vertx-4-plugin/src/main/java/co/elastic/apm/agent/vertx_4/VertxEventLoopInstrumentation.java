...
public abstract class VertxEventLoopInstrumentation extends VertxWebInstrumentation {
    ...
    public static class OnContextInstrumentation extends VertxEventLoopInstrumentation {
        @Override
        public ElementMatcher<? super TypeDescription> getTypeMatcher() {
            return hasSuperClass(named("io.vertx.core.impl.ContextImpl")).and(not(isInterface()));
        }

        @Override
        public ElementMatcher<? super MethodDescription> getMethodMatcher() {
            return named("runOnContext").and(takesArgument(0, named("io.vertx.core.impl.AbstractContext"))).and(takesArgument(1, named("io.vertx.core.Handler")));
        }
