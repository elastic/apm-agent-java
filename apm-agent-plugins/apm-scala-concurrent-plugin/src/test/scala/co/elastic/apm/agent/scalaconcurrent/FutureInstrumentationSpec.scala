...
class FutureInstrumentationSpec extends FunSuite {
  ...
  test("Scala Future should not propagate the tracing-context to unrelated threads") {
    ...
    val fs = (1 to 5).map(transactionNumber => Future {
      ...
      println(s"MILAN - thread=${Thread.currentThread().getId} transaction=$transactionNumber, span=${tracer.getActive}, trace=${tracer.currentTransaction()} starting transaction")
      ...
      val futures = (1 to 1)
        .map(futureNumber => Future {
          ...
          println(s"MILAN - thread=${Thread.currentThread().getId} transactionNumber=$transactionNumber, span=${tracer.getActive}. trace=${tracer.currentTransaction()} before futureNumber=$futureNumber, $currentTransactionNumber")
...
//
//          val currentTransactionNumber = tracer.currentTransaction().getNameAsString
//          println(s"MILAN - thread=${Thread.currentThread().getId} transactionNumber=$transactionNumber, span=${tracer.getActive}. trace=${tracer.currentTransaction()} map1 futureNumber=$futureNumber, $currentTransactionNumber")
//
//          assertEquals(transaction, tracer.currentTransaction())
//          assertEquals(currentTransactionNumber.toInt, transactionNumber)
//
