...
class JavaProcessBuilderApiInstrumentationTest extends AbstractInstrumentationTest {

    @BeforeEach
    void startTransaction() {
        Transaction transaction = tracer.startTransaction(TraceContext.asRoot(), null, null).activate();
        transaction.withName("Java Processbuilder api");
    ...
    void t() throws IOException, InterruptedException {
        ProcessBuilder javaVersion = new ProcessBuilder("java", "-version");
        ...
        assertThat(span.getSubtype()).isEqualTo("java processbuilder api");
