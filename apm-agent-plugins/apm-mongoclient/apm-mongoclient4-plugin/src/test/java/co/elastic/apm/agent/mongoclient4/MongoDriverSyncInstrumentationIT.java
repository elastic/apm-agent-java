...
public class MongoDriverSyncInstrumentationIT extends AbstractMongoDriverInstrumentationTest {
    ...
    public static void startMongoContainerAndClient() {
        mongo = MongoClients.create(new ConnectionString(String.format("mongodb://%s:%s",
            container.getContainerIpAddress(), container.getMappedPort(27017))));
    ...
    @Override
    public long count() {
        return db.getCollection(COLLECTION_NAME).estimatedDocumentCount();
    }
