...
public abstract class AbstractHttpClientInstrumentationTest extends AbstractInstrumentationTest {
        ...
        if (requestExecuted) {
            if (expectTraceContextHeaders) {
                ...
                findLoggedRequests(path).forEach(request ->
                    assertThat(TraceContext.containsTraceContextTextHeaders(TraceHeaderNameEncoding.REGULAR, request, HeaderAccessor.INSTANCE)).isFalse()
                );
            }
        }
    ...
    private void verifyTraceContextHeaders(Span span, String path) {
        ...
        loggedRequests.forEach(request -> {
            assertThat(TraceContext.containsTraceContextTextHeaders(TraceHeaderNameEncoding.REGULAR, request, HeaderAccessor.INSTANCE)).isTrue();
