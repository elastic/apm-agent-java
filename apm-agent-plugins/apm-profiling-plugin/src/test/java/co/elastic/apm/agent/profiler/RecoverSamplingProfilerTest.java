...
class RecoverSamplingProfilerTest {
    ...
    void testAll() throws Exception {
        when(profilingConfig.getRecoverMode()).thenReturn(31);
        verifyProfileTransaction();
    }

    @Test
    void testNone() throws Exception {
        when(profilingConfig.getRecoverMode()).thenReturn(0);
        verifyProfileTransaction();
    }

    @Test
    void test1() throws Exception {
        when(profilingConfig.getRecoverMode()).thenReturn(1);
        verifyProfileTransaction();
    }

    @Test
    void test2() throws Exception {
        when(profilingConfig.getRecoverMode()).thenReturn(2);
        verifyProfileTransaction();
    }

    @Test
    void test4() throws Exception {
        when(profilingConfig.getRecoverMode()).thenReturn(4);
        verifyProfileTransaction();
    }

    @Test
    void test8() throws Exception {
        when(profilingConfig.getRecoverMode()).thenReturn(8);
        verifyProfileTransaction();
    }

    @Test
    void test16() throws Exception {
        when(profilingConfig.getRecoverMode()).thenReturn(16);
        verifyProfileTransaction();
    }

    private void verifyProfileTransaction() throws Exception {
        ...
        Optional<Span> testProfileTransaction = reporter.getSpans().stream().filter(s -> s.getNameAsString().equals("RecoverSamplingProfilerTest#verifyProfileTransaction")).findAny();
        assertThat(testProfileTransaction).isPresent();
        assertThat(testProfileTransaction.get().isChildOf(transaction)).isTrue();

        Optional<Span> inferredSpanA = reporter.getSpans().stream().filter(s -> s.getNameAsString().equals("RecoverSamplingProfilerTest#aInferred")).findAny();
        ...
        Optional<Span> inferredSpanC = reporter.getSpans().stream().filter(s -> s.getNameAsString().equals("RecoverSamplingProfilerTest#cInferred")).findAny();
        assertThat(inferredSpanC).isPresent();
        assertThat(inferredSpanC.get().isChildOf(explicitSpanB.get())).isTrue();

        Optional<Span> inferredSpanD = reporter.getSpans().stream().filter(s -> s.getNameAsString().equals("RecoverSamplingProfilerTest#dInferred")).findAny();
