...
public class JakartaJmsInstrumentationHelper {
    ...
    private static final Logger logger = LoggerFactory.getLogger(JakartaJmsInstrumentationHelper.class);
    ...
    public Span startJmsSendSpan(Destination destination, Message message) {
        ...
        span.propagateTraceContext(message, JakartaJmsMessagePropertyAccessor.instance());
        if (span.isSampled()) {
            ...
            if (destinationName != null) {
                ...
                if (isDestinationNameComputed) {
                    JakartaJmsMessagePropertyAccessor.instance().setHeader(JMS_DESTINATION_NAME_PROPERTY, destinationName, message);
                }
            }
        }
        return span;
    ...
    public Transaction startJmsTransaction(Message parentMessage, Class<?> instrumentedClass) {
        Transaction transaction = tracer.startChildTransaction(parentMessage, JakartaJmsMessagePropertyAccessor.instance(), instrumentedClass.getClassLoader());
        if (transaction != null) {
    ...
    public void makeChildOf(Transaction childTransaction, Message parentMessage) {
        TraceContext.<Message>getFromTraceContextTextHeaders().asChildOf(childTransaction.getTraceContext(), parentMessage, JakartaJmsMessagePropertyAccessor.instance());
    }
    ...
    public MessageListener wrapLambda(@Nullable MessageListener listener) {
        ...
        if (listener != null && listener.getClass().getName().contains("/")) {
            return new JakartaJmsInstrumentationHelper.MessageListenerWrapper(listener);
        }
        return listener;
    }
