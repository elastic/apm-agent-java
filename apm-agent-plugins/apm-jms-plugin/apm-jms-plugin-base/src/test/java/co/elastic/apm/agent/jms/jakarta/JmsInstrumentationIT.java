...
import co.elastic.apm.agent.jms.jakarta.JmsInstrumentationHelper;
...
public class JmsInstrumentationIT extends AbstractInstrumentationTest {
    ...
    public static Iterable<Object[]> brokerFacades() {
        return Arrays.asList(new Object[][]{{new ActiveMqArtemisFacade(), ActiveMqArtemisFacade.class}});
    }
    ...
    private void verifySendListenOnNonTracedThread(String destinationName, TextMessage message, int expectedReadTransactions) throws JMSException {
        ...
        for (Transaction receiveTransaction : receiveTransactions) {
            ...
            assertThat(receiveTransaction.getType()).isEqualTo(co.elastic.apm.agent.jms.jakarta.JmsInstrumentationHelper.MESSAGING_TYPE);
    ...
    private void verifyMessageHeaders(Message message, Transaction receiveTransaction) throws JMSException {
        ...
        assertThat(message.getJMSMessageID()).isEqualTo(headersMap.get(co.elastic.apm.agent.jms.jakarta.JmsInstrumentationHelper.JMS_MESSAGE_ID_HEADER));
        assertThat(String.valueOf(message.getJMSExpiration())).isEqualTo(headersMap.get(co.elastic.apm.agent.jms.jakarta.JmsInstrumentationHelper.JMS_EXPIRATION_HEADER));
        assertThat(String.valueOf(message.getJMSTimestamp())).isEqualTo(headersMap.get(co.elastic.apm.agent.jms.jakarta.JmsInstrumentationHelper.JMS_TIMESTAMP_HEADER));
    ...
    public void testTibcoTempQueueListener() {
        try {
            testQueueSendListen(brokerFacade.createQueue(co.elastic.apm.agent.jms.jakarta.JmsInstrumentationHelper.TIBCO_TMP_QUEUE_PREFIX + UUID.randomUUID().toString()),
    ...
    private void testQueueSendListen(Queue queue, Function<Destination, CompletableFuture<Message>> listenerRegistrationFunction)
        ...
        if (queue instanceof TemporaryQueue || queueName.startsWith(co.elastic.apm.agent.jms.jakarta.JmsInstrumentationHelper.TIBCO_TMP_QUEUE_PREFIX)) {
